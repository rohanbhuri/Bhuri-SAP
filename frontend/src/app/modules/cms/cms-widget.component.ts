...
@Component({
  ...
  template: `
    <div class="cms-widget">
      <div class="widget-header-content">
        <p class="subtitle">Content management for blogs & news</p>
      </div>
        ...
        <div class="widget-stats">
          <div class="stat-item primary">
            ...
            <div class="stat-info">
              ...
              <span class="stat-detail">{{ stats().publishedBlogs }} published</span>
            </div>
          </div>
          <div class="stat-item secondary">
            <mat-icon>newspaper</mat-icon>
            <div class="stat-info">
              <span class="stat-number">{{ stats().newsMedia }}</span>
              <span class="stat-label">News & Media</span>
              <span class="stat-detail">{{ stats().publishedNews }} published</span>
            </div>
          </div>
      ...
      <div class="widget-footer-actions">
        <div class="cta-grid">
          <button mat-flat-button class="cta-btn" (click)="navigateToTab('blogs')">
            <mat-icon>article</mat-icon>
            <span>Blogs & Articles</span>
          </button>
          <button mat-flat-button class="cta-btn" (click)="navigateToTab('news-media')">
            <mat-icon>newspaper</mat-icon>
  ...
  styles: [`
    ...
    .stat-item.secondary { border-left: 3px solid #ff6b6b; }
    .stat-item.draft { border-left: 3px solid #ffa502; }
    .stat-item.total { border-left: 3px solid #a29bfe; }
  `]
})
export class CmsWidgetComponent implements OnInit {
  ...
  loadStats() {
    ...
    // Load news & media
    this.cmsService.getNewsMedia().subscribe(newsMedia => {
      const published = newsMedia.filter(n => n.status === 'published').length;
      const drafts = newsMedia.filter(n => n.status === 'draft').length;
      totalDrafts += drafts;
      totalContent += newsMedia.length;
  ...
  navigateToTab(tabName: string) {
    this.router.navigate(['/modules/cms'], { queryParams: { tab: tabName } });
  }
}