...
async function runTests() {
    ...
    try {
        ...
        let res = await makeRequest('GET', '/pages');
        if (res.status === 401) {
        ...
        res = await makeRequest('GET', '/pages', null, { 'X-API-Key': 'invalid-key' });
        if (res.status === 401) {
            console.log('✅ PASS: Got 401 Unauthorized\n');
            passed++;
        } else {
        ...
        // Test 5: Create page with API key
        console.log('Test 5: Create page with API key');
        console.log('⚠️  SKIPPED: Requires valid API key\n');

        // Test 6: Get pages with API key
        console.log('Test 6: Get pages with API key');
        console.log('⚠️  SKIPPED: Requires valid API key\n');
        ...
        console.log('5. Run: node test-cms-api-key.js\n');
